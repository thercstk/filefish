...
def _encode_move(src, dst, flags, moved):
...
def generate_moves(board: Board) -> list:
    ...
    while d_pushes:
        ...
        d_pushes &= d_pushes - 1
        move = _encode_move(sq + D_PUSH[turn], sq, double_pawn, PAWNS)
        movelist.append(move)

    while attacks_left_np:
        sq = (attacks_left_np & -attacks_left_np).bit_length()-1
        attacks_left_np &= attacks_left_np - 1
        move = _encode_move(sq + ATK_R[1-turn], sq, capture, PAWNS)
        movelist.append(move)
    ...
    while attacks_right_p:
        ...
        for flag in range(12, 16):
            move = _encode_move(sq + ATK_L[1-turn], sq, flag, PAWNS)
            movelist.append(move)

    while pushes_np:
        sq = (pushes_np & -pushes_np).bit_length()-1
        pushes_np &= pushes_np - 1
        move = _encode_move(sq + PUSH[turn], sq, quiet_move, PAWNS)
        movelist.append(move)
    ...
    while pushes_p:
        ...
        for flag in range(8, 12):
            move = _encode_move(sq + PUSH[turn], sq, flag, PAWNS)
            movelist.append(move)

    if passant_left:
        sq = (passant_left & -passant_left).bit_length()-1
        move = _encode_move(sq + ATK_R[1-turn], sq, en_passant, PAWNS)
        movelist.append(move)

    if passant_right:
        sq = (passant_right & -passant_right).bit_length()-1
        move = _encode_move(sq + ATK_L[1-turn], sq, en_passant, PAWNS)
        movelist.append(move)
